     ...
     *  nt,tt,np,tp,c,fp,wrk1,lwrk1,wrk2,lwrk2,iwrk,kwrk,tol,maxit,ier)
      implicit none
...
c  set constants
      one = 0.1e+01
c  tol and maxit are DUMMY ARGUMENTS in this build. scijit fork of the
c  vendored Dierckx source, which set them here as locals:
c      maxit = 20
c      tol = 0.1e-02
c  scijit.interpolate.fitters resolves its sentinel defaults to exactly
c  those two values, so an unchanged call is an unchanged computation.
