...
!  * QUADPACK: date written 800101, revision date 830518 (yymmdd)
    ...
    subroutine dqawo(f, a, b, Omega, Integr, Epsabs, Epsrel, Result, Abserr, &
        ...
        integer, intent(out), optional :: Momcom !! on return, the number of chebyshev
                                                 !! moments computed, which is what
                                                 !! [[dqawoe]] reports and this driver
                                                 !! otherwise discards. `0` when the
                                                 !! `leniw`/`maxp1`/`lenw` check ends the
                                                 !! call before [[dqawoe]] is entered.
                                                 !! LOCAL ADDITION to the vendored
                                                 !! source, optional so every existing
                                                 !! caller is unaffected. Its only user
                                                 !! is `dqawo_wrapper` in wrappers.f90.

        integer :: limit, lvl, l1, l2, l3, l4, mcom
        ...
        if (Leniw >= 2 .and. Maxp1 >= 1 .and. Lenw >= (Leniw*2 + Maxp1*25)) then
            ...
            call dqawoe(f, a, b, Omega, Integr, Epsabs, Epsrel, limit, 1, Maxp1, &
                        ...
                        Work(l2), Work(l3), Iwork(1), Iwork(l1), mcom, &
                        Work(l4))
            ! call error handler if necessary
            lvl = 0
        end if
        if (present(Momcom)) Momcom = mcom
        if (Ier == 6) lvl = 1
