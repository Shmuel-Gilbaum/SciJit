...
!    R.H. Byrd, P. Lu-Chen and J. Nocedal.
      ...
      integer,intent(in),optional :: Maxls !! SCIJIT PATCH: maximum number of
                                           !! line-search steps per iteration.
                                           !! Absent -> 20, the value this code
                                           !! hardcoded and the same default
...
!    Written by Ciyou Zhu in collaboration with R.H. Byrd, P. Lu-Chen and J. Nocedal.
      ...
      integer :: maxls_   ! SCIJIT PATCH: resolved Maxls, 20 when absent
      ...
      ! SCIJIT PATCH: resolve Maxls every entry, not only at 'START'.  setulb
      ! is REVERSE COMMUNICATION, so mainlb is re-entered on each callback and
      ! every entry must see the caller's value.
      maxls_ = 20
      if ( present(Maxls) ) maxls_ = Maxls
      if ( maxls_ < 1 ) maxls_ = 1
      ...
      main_loop : do
         ...
         if (linesearch) then
            ...
            if ( info/=0 .or. iback>=maxls_ ) then
               ! restore the previous iterate.
               call dcopy(n,t,1,x,1)
               call dcopy(n,r,1,g,1)
               f = fold
